...
namespace SkyLearnApi.Services.Implementation
{
    public class EmailJobs
    {
        ...
        public async Task SendWelcomeEmailAsync(string email, string fullName, string role)
        {
            ...
            {
                _logger.LogWarning("SendWelcomeEmailAsync called with empty email. FullName: {FullName}, Role: {Role}", fullName, role);
                return;
            }

            _logger.LogInformation("Attempting to send welcome email to {Email}. FullName: {FullName}, Role: {Role}", email, fullName, role);
            ...
            {
                _logger.LogError(ex,
                    "FAILED to send welcome email to {Email}. ExceptionType: {ExceptionType}, Message: {Message}",
                    email, ex.GetType().Name, ex.Message);
                throw; // Re-throw so Hangfire retries (up to 3 attempts with increasing delays)
            }
        }
    }
}
