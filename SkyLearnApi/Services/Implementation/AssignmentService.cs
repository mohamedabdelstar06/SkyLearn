...
namespace SkyLearnApi.Services.Implementation
{
    public class AssignmentService : IAssignmentService
    {
        ...
        {
            _logger.LogInformation("Creating assignment in course {CourseId}. Title: {Title}, UserId: {UserId}, Start: {Start}, DeadLine: {DeadLine}",
                courseId, dto.Title, userId, dto.StartDate, dto.DeadLineDate);
        ...
        public async Task<AssignmentResponseDto?> UpdateAsync(int id, UpdateAssignmentDto dto, int userId)
        {
            ...
            if (dto.DeadLineDate.HasValue) assignment.DeadLineDate = dto.DeadLineDate.Value;
            
            // Validate dates
            if (assignment.DeadLineDate <= assignment.StartDate)
                throw new ArgumentException("DeadLineDate must be greater than StartDate.");
        ...
        public async Task<AssignmentSubmissionResponseDto> SubmitAsync(int assignmentId, IFormFile file, int studentId)
        {
            ...
            if (assignment.StartDate.HasValue && now < assignment.StartDate.Value)
                throw new ArgumentException($"This assignment is not available for submission until {assignment.StartDate.Value:g}.");

            if (assignment.DeadLineDate.HasValue && now > assignment.DeadLineDate.Value)
                throw new ArgumentException("The absolute cutoff date for this assignment has passed. Submissions are no longer accepted.");

            // Late submissions are functionally blocked after DeadLineDate
            bool isLate = false;
        ...
        public async Task<AssignmentSubmissionResponseDto> GradeAsync(int assignmentId, int studentId, GradeAssignmentDto dto, int graderId)
        {
            ...
            // Explicitly load GradedBy so the name is available in the response
            await _context.Entry(submission).Reference(s => s.GradedBy).LoadAsync();
