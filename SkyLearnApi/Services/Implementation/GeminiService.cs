...
{content}";
        ...
        public async Task<string> GenerateChatResponseAsync(string systemPrompt, List<SkyLearnApi.DTOs.Chat.ChatMessageDto> history, string newMessage, CancellationToken cancellationToken = default)
        {
            var url = $"https://generativelanguage.googleapis.com/v1beta/models/{_settings.Model}:generateContent?key={_settings.ApiKey}";

            // Build history format required by Gemini
            ...
            for (int attempt = 0; attempt < _settings.MaxRetries; attempt++)
            {
                try
                {
                    ...
                    {
                        _logger.LogWarning("Gemini API Chat error (attempt {Attempt}): {Status} {Body}", attempt + 1, response.StatusCode, responseBody);
                        if (attempt == _settings.MaxRetries - 1) throw new Exception($"Gemini API failed: {responseBody}");
