...
namespace SkyLearnApi.Controllers
{
    ...
    public class QuizzesController : ControllerBase
    {
        ...
        [HttpPost("quizzes/{id}/auto-save")]
        [Authorize(Roles = Roles.Student)]
        public async Task<IActionResult> AutoSave(int id, [FromBody] QuizAutoSaveDto dto)
        {
            if (!UserId.HasValue) return Unauthorized();
            try
            {
                var result = await _quizService.AutoSaveProgressAsync(id, dto, UserId.Value);
                if (!result) return BadRequest(new { message = "Failed to auto-save. The attempt may have expired or does not exist." });
