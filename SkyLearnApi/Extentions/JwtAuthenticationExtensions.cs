            ...
            .AddJwtBearer(options =>
            {
                ...
                options.Events = new JwtBearerEvents
                {
                    ...
                    OnAuthenticationFailed = context =>
                    {
                        ...
                        // Strip ALL control characters (newlines, tabs, carriage returns, etc.)
                        // to prevent "Invalid control character in header" 400 errors
                        var cleanError = new string(error.Where(c => c >= 32).ToArray());
                        if (cleanError.Length > 200) cleanError = cleanError.Substring(0, 200);
